@.geo-topic-agent-runtime/AGENTS.md
...
Run CLI commands through `.geo-topic-agent-runtime/geo_agent_cli.py` so a fresh process does not depend on `PYTHONPATH`. First setup command: `python .geo-topic-agent-runtime/geo_agent_cli.py setup --project-dir "." --runtime claude`.
